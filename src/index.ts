...
class Launcher {
  constructor(ctx: Context, config: Launcher.Config) {
      ...
      ctx.command('欢迎使用早苗~').action(() => {
        // 输出当前的配置
        //return `连接到: ${config.path}\n端口: ${config.port}`
      })
...
namespace Launcher {
  ...
  export const Config: Schema<Config> = Schema.object({
    path: Schema.string().role('link').description('早苗服务器的地址,由于本插件本身是一个转接器,所以也可以是其他的ob11应用端(开始套娃!)').default('ws://101.35.247.237'),
    port: Schema.string().description('早苗服务器的端口,范围20001~20150,每个端口对应不同的存档,所以请牢记地址与端口的组合,遗忘会丢失早苗存档.请手动指定一个数字,而不要都使用20002').default('20002'),
    type: Schema.string().description('20001~20050,早苗,20050~20070,澪20071~20099,浅羽,20099-20120浅羽,0120-20150澪').default('端口定义'),
    log:  Schema.boolean().default(false).description('日志开关,上面的设置每次修改后,重启koishi才会生效.'),
    info:  Schema.string().role('link').description('这个插件可以结合存储库另一个项目载入epl插件(mpq-sdk)右边是指令文档').default('yuque.com/km57bt/hlhnxg/'),
  })
}
 ...
 * 分发事件
 */
 export function dispatch(event: string,bot: Bot,config: Launcher.Config): void {
...
function adaptData(session: Session): any {
  const data: any = {
  ...
  if (session.type === "message" || session.type === "message_sent") {
    ...
    if (session.type === "message_sent" && session.channelId?.startsWith("private:")) {
      data.target_id = session.channelId.substr(8);
    }
    ...
    data.message_seq = 0;
    data.anonymous = null; 
    data.user_id = session.author.userId; 
